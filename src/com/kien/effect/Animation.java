...
public class Animation {
    private String name; // tên các animation

    private boolean isRepeated; //kiểm tra: nếu false thì animation ko lặp lại

    private ArrayList<FrameImage> frameImages;
    private int currentFrame; //ám chỉ hình đang đc vẽ trên màn hình

    private ArrayList<Boolean> ignoreFrames; // kiểm tra để bỏ các hình ko cần(vd động tác chạy, khi đang chạy ko còn hành động đứng yên)

    private ArrayList<Double> delayFrames; // thời gian hiện giữa các frame

    private long beginTime;

    private boolean drawRectFrame; // ẩn hiện khung quanh frame con
    ...
    public boolean isIgnoreFrame(int id) { // kiểm tra frame này xem có phải bỏ qua ko
    ...
    private void nextFrame() {
        ...
        if (currentFrame >= frameImages.size() - 1) { //kiểm tra đến frame cuối trong mảng chưa
            if (isRepeated) currentFrame = 0;        //nếu true là vòng lặp thì bắt đầu lại vòng lặp
        } else currentFrame++;

        if (ignoreFrames.get(currentFrame)) {    //nếu là 1 frame bị bỏ qua thì next frame khác
            nextFrame();
        }

    }

    public boolean isLastFrame() { //kiểm tra animation này chạy xong chưa
        if (currentFrame == frameImages.size() - 1)
            return true;
        else return false;
    }

    public void flipAllImage() {   //lật ngược frame lại
