...
public class Megaman extends Human {
    ...
    public void Update() {
        ...
        if (isShooting) {
            if (System.nanoTime() - lastShootingTime > 500 * 1000000) {  //ktra nửa s thì đc bắn tiếp
                isShooting = false;
            }
        }
    ...
    public void draw(Graphics2D g2) {
        ...
        switch (getState()) {
            ...
            case ALIVE:
            case NOBEHURT:
                if (getState() == NOBEHURT && (System.nanoTime() / 10000000) % 2 != 1) {  //trạng thái bất tử nhân vật bị nhấp nháy
    ...
    public void jump() {
        ...
        if (!getIsJumping()) {   //nếu ko phải đang bay
            setIsJumping(true);
            setSpeedY(-5.0f);
            flyBackAnim.reset();
    ...
    public void attack() {
        ...
        if (!isShooting && !getIsDicking()) { //nếu đang bắn thì ko dc bắn tiếp or ngồi thì ko đc bắn
